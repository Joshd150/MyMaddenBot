...
export function TeamPlayersDialog({
  ...
  const [sortColumn, setSortColumn] = useState<string>("overall")
  ...
  // Extra check: fallback to empty array if allTeams is undefined
  const safeTeams = Array.isArray(allTeams) ? allTeams : []
  const safePlayers = Array.isArray(allPlayers) ? allPlayers : []

  const team = useMemo(
    () => safeTeams.find((t) => t.teamId === teamId),
    [teamId, safeTeams]
  )

  const playersOnTeam = useMemo(() => {
    return safePlayers.filter((player) => player.teamId === teamId)
  }, [teamId, safePlayers])
  ...
  // If team isn't found, don't render dialog
  if (!team) {
    return null
  }
